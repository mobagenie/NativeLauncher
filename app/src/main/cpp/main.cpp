...
void loadApps(){
    ...
    if(apps.empty()){
        apps={{"Settings","com.android.settings"},
...
void drawText(uint32_t* p,int w,int x,int y,const char* t){
    if(!fontReady) return;
    float s=stbtt_ScaleForPixelHeight(&font,20); // lebih kecil
    int cx=x;
    while(*t){
        int ax,l;
        ...
        for(int j=0;j<bh;j++)
            for(int i=0;i<bw;i++)
                if(b[j*bw+i])
                    if((y+j)>=0 && (y+j)<buf.height && (cx+i)<w) // safe check
...
void draw(){
    ...
    int cols=4; // grid lebih rapat
    int cw=buf.width/cols;

    int rowHeight=160; // lebih pendek
    int contentHeight=((apps.size()+cols-1)/cols)*rowHeight;

    // clamp scroll
    if(scrollY>0) scrollY=0;
    if(scrollY<buf.height-contentHeight) scrollY=buf.height-contentHeight;

    for(int i=0;i<apps.size();i++){
        int r=i/cols;
        int c=i%cols;
        int sx=c*cw+10;
        int sy=r*rowHeight+20 + scrollY;

        if(sy+rowHeight<0 || sy>buf.height) continue; // culling

        // box icon
        int boxW=cw-20;
        int boxH=rowHeight-40;
        for(int y=sy;y<sy+boxH;y++)
            for(int x=sx;x<sx+boxW;x++)
                if(y>=0 && y<buf.height && x<buf.width)
                    p[y*buf.stride+x]=0xffffffff;

        drawText(p,buf.stride,sx+5,sy+boxH-30,apps[i].name.c_str());
    }
...
int32_t input(android_app* app,AInputEvent* ev){
    ...
    if(act==AMOTION_EVENT_ACTION_UP){
        ...
        for(int i=0;i<apps.size();i++){
            ...
            int sx=c*cw+10;
            int sy=r*rowHeight+20+scrollY;
            if(x>=sx && x<=sx+cw-20 && y>=sy && y<=sy+rowHeight-40)
